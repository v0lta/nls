...
for kindex=1:kkmax
  ...
  ss=size(xx);
  var=xx(ss(1),:)'; %ss(end,:)' does the same thing and is easier to read!
                    % anyway this command gets the output which is realted
                    % to the Jacobian!
  clear t xx
  %
  ...
  %
  x=var(1:n);
  dx=reshape(var(n+1:length(var)),n,n); %assemble the jacobian
    %
