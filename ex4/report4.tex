...
In this section the topology of the first interval $c \in [0,0.4]$ will be deduced from the eigenvalues and eigenvectors. For $\mathbf{x}_1^*$ reading off from \ref{eq:Jx1} the eigenvalues are found to be $\lambda_{1,1} =  -a$, $\lambda_{1,2} =  -c$, with the eigenvectors $\mathbf{v}_{1,1} = \begin{pmatrix} 1 & 0 \end{pmatrix}^T$, $\mathbf{v}_{1,2} = \begin{pmatrix} 0 & 1 \end{pmatrix}^T$.

For $\mathbf{x}_2^*$ no general expressions could be found from~\ref{eq:Jx2}, the ones given are for $c = 0.2, a = 0.4$ and $b = 0.3$. 
$\lambda_{2,1} = 0.305$, $\lambda_{2,2} = -1.04$,  $ \mathbf{v}_{2,1} = \begin{pmatrix} -0.571 & 1 \end{pmatrix}^T$, $ \mathbf{v}_{2,2} = \begin{pmatrix} 0.1967 & 1 \end{pmatrix}^T$.

For $\mathbf{x}_3^*$ the eignevalues of the Jacobian~\ref{eq:Jx3} may be read of the diagonal: $\lambda_{3,1} = a+1$, $\lambda_{3,2} = 1-c$. With the eigenvalues known the eigenvectors may be  computed, they turn out to be: $ \mathbf{v}_{3,1} = \begin{pmatrix} 1 & 0 \end{pmatrix}$, $\mathbf{v}_{3,2} = \begin{pmatrix} 1 & -\frac{a+c}{b} \end{pmatrix}$.

Finally for $\mathbf{x}_4^*$ is very similar from~\ref{eq:Jx4}, $\lambda_{4,1} = -a^2 + a$ and $\lambda_{4,2} = a-c$ are the read off eigenvalues, thus the eigenvectors are $ \mathbf{v}_{4,1} = \begin{pmatrix} 1 & 0 \end{pmatrix}^T$, $ \mathbf{v}_{4,2} = \begin{pmatrix} 1 & \frac{-a^2+c}{ba} \end{pmatrix}^T$
