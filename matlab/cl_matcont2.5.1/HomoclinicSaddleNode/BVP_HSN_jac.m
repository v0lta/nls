...
for j=1:homds.nphase+2
   ...
   %     for i=1:homds.nneg+1
            result(homds.ncoords+2+1:homds.ncoords+2+1+homds.npos*(homds.nneg+1),homds.ncoords+j) = tmp(:);
   %    end    
    end
    if ~isempty(YS)
...
if homds.nneg
    ...
    for i=1:homds.nneg
        result(end-1-homds.npos-homds.nphase+i,indxs) = [Qn(:,end-i+1)'     -Qn(:,end-i+1)'];
    end
    for i=1:homds.nneg
        vQb = vect' * dQtot{i};
        result(end-1-homds.npos-homds.nphase+i,homds.ncoords+1:homds.ncoords+homds.nphase+2) = ...
            result(end-1-homds.npos-homds.nphase+i,homds.ncoords+1:homds.ncoords+homds.nphase+2) + ...
            vQb;
    end
    if homds.npos
        for j=1:homds.npos+1
            result(end-homds.nphase-homds.npos:end-homds.nphase-homds.npos-1+homds.nneg,end-homds.Ysize+(j-1)*(homds.npos)+1:end-homds.Ysize+j*(homds.npos)) = vQ(:,j) .* eye(homds.nneg);
        end
    end
end
...
if homds.npos
    ...
    for i=1:homds.npos
        result(end-homds.nphase-homds.npos+homds.nneg+i,indxs) = [Qn(:,end-i+1)'     -Qn(:,end-i+1)'];
    end
    for i=1:homds.npos
        vQb = vect' * dQtot{i};
        result(end-homds.nphase-homds.npos+homds.nneg+i,homds.ncoords+1:homds.ncoords+homds.nphase+2) = ...
            result(end-homds.nphase-homds.npos+homds.nneg+i,homds.ncoords+1:homds.ncoords+homds.nphase+2) + ...
           vQb;
    end    
    if homds.nneg
        for j=1:homds.nneg+1
            result(end-homds.nphase-homds.npos+homds.nneg:end-2,end-(homds.npos+1)*(homds.nneg)+(j-1)*homds.npos+1:end-(homds.npos+1)*(homds.nneg)+j*homds.npos) = vQ(:,j) * eye(homds.nneg);
        end
    end
end
