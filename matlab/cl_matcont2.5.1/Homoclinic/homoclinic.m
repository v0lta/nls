...
%        LUorth = Q0U * [-YU'; eye(size(YU,1))]
     %%kolommen anders niet in goede volgorde!
        ...
        res(13,1) = exp(-D1(mu1) * T) * (v1s' * LUorth' * sigma2);
...
%        LSorth = Q0S * [-YS'; eye(size(YS,1))]
        ...
        res(14,1) = exp(D1(lambda1) * T) * (v1u' * LSorth' * sigma1);
...
x(end-size(homds.YS)-size(homds.YU)+1:end)=zeros(size(homds.YS)+size(homds.YU),1);
